...
const getAirQualityInfo = (index) => {
  switch (index) {
    case 1:
      return {
        label: "Excelente",
        description: "Aire puro y saludable",
        color: "#10b981",
    ...
    case 3:
      return {
        label: "Moderada",
        description: "Sensible para grupos de riesgo",
        color: "#f59e0b",
        bgColor: "bg-amber-50",
        emoji: "😐",
      };
    case 4:
      return {
        label: "Pobre",
        description: "Riesgo para toda la población",
        color: "#f97316",
    ...
    case 5:
      return {
        label: "Muy Pobre",
        description: "Alerta de salud pública",
        color: "#ef4444",
        bgColor: "bg-red-50",
        emoji: "🤒",
      };
...
const getMoonPhase = (phase) => {
  ...
  if (phase.includes("Waxing") || phase.includes("First"))
    return "Cuarto creciente";
  if (phase.includes("Waning") || phase.includes("Last"))
...
const getMoonIcon = (phase) => {
  if (phase.includes("New"))
    return <WiMoonAltNew className="text-2xl text-blue-300" />;
  if (phase.includes("Full"))
    return <WiMoonAltFull className="text-2xl text-yellow-300" />;
  if (phase.includes("Waxing") || phase.includes("First"))
    return <WiMoonAltWaxingCrescent4 className="text-2xl text-gray-300" />;
  return <WiMoonAltWaningCrescent3 className="text-2xl text-gray-400" />;
};

const getTemperatureRange = (min, max, current) => {
  const range = max - min;
  if (range === 0) return 50;
  const currentPos = ((current - min) / range) * 100;
  return Math.min(100, Math.max(0, currentPos));
};
        ...
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-6">
            <div className="relative">
              <img
                src={`https:${current.condition.icon.replace(
                  "64x64",
                  "128x128"
                )}`}
          ...
          {/* Barra de temperatura con degradado mixto */}
          <div className="w-full">
            <div className="flex justify-between text-sm text-gray-600 mb-1">
              <span className="whitespace-nowrap">
                Mín: {todayForecast.mintemp_c}°C
              </span>
              <span className="whitespace-nowrap">
                Media:{" "}

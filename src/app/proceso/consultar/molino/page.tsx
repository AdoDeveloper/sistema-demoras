...
// Para el modal, se omiten "id", "createdAt", "updatedAt" y llaves de relación
function filterModalDetailData(data: any) {
  if (!data) return data;
  const excludeKeys = ["id", "createdAt", "updatedAt", "parosMol", "vueltasMol", "molId", "segundoProcesoMolId", "tercerProcesoMolId"];
  return Object.fromEntries(Object.entries(data).filter(([key]) => !excludeKeys.includes(key)));
}
      ...
      {paros.map((paro, index) => (
        <div key={paro.id} className="mb-2 border rounded">
          ...
          <table className="w-full text-xs border-collapse">
            <tbody>
              {Object.entries(paro)
                .filter(([key]) => !["id", "createdAt", "updatedAt", "segundoProcesoMolId"].includes(key))
                .map(([key, value]) => (
          ...
          <table className="w-full text-xs border-collapse">
            <tbody>
              {Object.entries(vuelta)
                .filter(([key]) => !["id", "createdAt", "updatedAt", "tercerProcesoMolId"].includes(key))
                .map(([key, value]) => (
                  <tr key={key} className="border-b">
                    <td className="px-2 py-1 font-bold bg-blue-50 whitespace-nowrap">{formatKey(key)}</td>
                    <td className="px-2 py-1 whitespace-nowrap">{displayValue(value)}</td>
      ...
      if (Array.isArray(value) && (key === "parosMol" || key === "vueltasMol")) {
        ...
        value.forEach((item: any, index: number) => {
          ...
          for (const [k, v] of Object.entries(item)) {
            if (["id", "createdAt", "updatedAt", "segundoProcesoMolId", "tercerProcesoMolId"].includes(k)) continue;
...
const handleDescargarPDF = async (selectedDemora: any) => {
  ...
  try {
    ...
    if (selectedDemora.tercerProceso) {
      ...
      if (selectedDemora.tercerProceso.vueltasMol && selectedDemora.tercerProceso.vueltasMol.length > 0) {
        addTableSection("Tercer Proceso - Vueltas", { vueltasMol: selectedDemora.tercerProceso.vueltasMol });
      }
    }
...
export default function DemorasPage() {
  ...
  const filteredDemoras = demoras.filter((item) => {
    ...
    if (filterTransaccion) {
      if (!(item.primerProceso && item.primerProceso.numeroTransaccion && item.primerProceso.numeroTransaccion.toString().toLowerCase().includes(filterTransaccion.toLowerCase()))) return false;
    }
    if (filterOrden) {
      if (!(item.primerProceso && item.primerProceso.numeroOrden && item.primerProceso.numeroOrden.toString().toLowerCase().includes(filterOrden.toLowerCase()))) return false;
    }
    if (filterCondicion) {
    ...
    if (filterMolino && filterMolino !== "Todos") {
      if (!(item.primerProceso && item.primerProceso.numeroMolino && item.primerProceso.numeroMolino.toLowerCase().includes(filterMolino.toLowerCase()))) return false;
    }
    if (filterCriba && filterCriba !== "Todos") {
      if (!(item.primerProceso && item.primerProceso.numeroCriba && item.primerProceso.numeroCriba.toLowerCase().includes(filterCriba.toLowerCase()))) return false;
    }
    if (filterPresentacion && filterPresentacion !== "Todos") {
      if (!(item.primerProceso && item.primerProceso.presentacion && item.primerProceso.presentacion.toLowerCase() === filterPresentacion.toLowerCase())) return false;
    }
    if (fechaInicio) {
      ...
      <header className="bg-gradient-to-r bg-orange-400 text-white shadow-lg md:sticky md:top-0 z-50">
        <div className="mx-auto px-4 py-4">
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col">
              <label className="text-sm">Molino</label>
              <select
                value={filterMolino}
                onChange={(e) => setFilterMolino(e.target.value)}
                className="text-black px-2 py-1 rounded"
              >
            ...
            <div className="flex flex-col">
              <label className="text-sm">Criba</label>
              <select
                value={filterCriba}
                onChange={(e) => setFilterCriba(e.target.value)}
                className="text-black px-2 py-1 rounded"
              >
                <option value="Todos">Todos</option>

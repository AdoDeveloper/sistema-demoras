...
export default function ProcesoFinal() {
  ...
  // Almacenará toda la información (resumen) de "envasadoProcess"
  ...
  const guardarDatosProcesoFinal = () => {
    ...
    if (!stored) {
      console.error("No se encontró envasadoProcess en localStorage");
      return;
    }
    const parsed = JSON.parse(stored);
  ...
  const handleGuardarLocal = () => {
    const stored = localStorage.getItem("envasadoProcess");
    if (!stored) {
      Swal.fire("Error", "No se encontró envasadoProcess en localStorage", "error");
      return;
    }
    const parsed = JSON.parse(stored);
    if (!parsed || typeof parsed !== "object") {
  ...
  const handleEnviar = async () => {
    ...
    if (confirmResult.isConfirmed) {
      ...
      try {
        ...
        console.log("Payload a enviar:", JSON.stringify({ envasadoProcess: parsed }, null, 2));

        const res = await fetch("/api/demoras/envasado", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ envasadoProcess: parsed }),
        });
        Swal.close();
        if (res.ok) {

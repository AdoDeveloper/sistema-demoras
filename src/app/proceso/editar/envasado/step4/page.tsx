...
export default function ProcesoFinal() {
  ...
  // Almacenará la información del editEnvasado (payload sin la fecha de salida)
  ...
  // Cargar la data de "editEnvasado" y de "fechaSalidaPlanta" desde localStorage
  ...
  const generateReadableNote = (data: any): string => {
    ...
    if (Array.isArray(segundo.paros)) {
      ...
      segundo.paros.forEach((p, idx) => {
        note += `      Paro ${idx + 1}: Inicio ${p.inicio}, Fin ${p.fin}, Razón: ${p.razon}, Diff: ${p.diffCargaInicio}, Duración: ${p.duracionParo}\n`;
      });
    }
  ...
  // Actualiza el objeto editEnvasado sin la fecha de salida y lo guarda en localStorage
  const guardarDatosProcesoFinal = () => {
    ...
    localStorage.setItem("editEnvasado", JSON.stringify(updated));
  };
  ...
  const handleGuardarLocal = () => {
    ...
    if (!stored) {
      Swal.fire("Error", "No se encontró editEnvasado en localStorage", "error");
      return;
    }
    const parsed = JSON.parse(stored);
  ...
  const handleEnviar = async () => {
    ...
    if (confirmResult.isConfirmed) {
      ...
      try {
        ...
        const stored = localStorage.getItem("editEnvasado");
        if (!stored) throw new Error("No se encontró editEnvasado en storage");
        const payload = JSON.parse(stored);
        console.log("Payload enviado:", JSON.stringify(payload, null, 2));
        const envasadoId = localStorage.getItem("envasadoId") || "0";
        const res = await fetch(`/api/demoras/envasado/${envasadoId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ editEnvasado: payload }),
        });
        Swal.close();
        if (res.ok) {

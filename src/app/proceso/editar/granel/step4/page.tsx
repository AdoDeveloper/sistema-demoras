...
export default function ProcesoFinal() {
  ...
  // Almacenará la información del editDemora (payload sin la fecha de salida)
  ...
  // Cargar la data de "editDemora" y de "fechaSalidaPlanta" desde localStorage
  ...
  const generateReadableNote = (data: any): string => {
    ...
    note += `Registro editado N°: ${localStorage.getItem("demoraId")}\n`;
    ...
    // Se muestra la fecha en el TXT para información (aunque no se envía en el payload)
    note += `  Salida de Planta: ${tiempoSalidaPlanta.fecha || "N/A"} ${final.tiempoSalidaPlanta?.hora || "N/A"} - ${final.tiempoSalidaPlanta?.comentarios || ""}\n`;
    note += `  Portería Salida: ${final.porteriaSalida || "N/A"}\n\n`;
    note += `Tiempo Total: ${data.tiempoTotal || "N/A"}\n`;
    return note;
  };

  // Actualiza el objeto editDemora sin la fecha de salida y lo guarda en localStorage
  const guardarDatosProcesoFinal = () => {
    ...
    localStorage.setItem("editDemora", JSON.stringify(updated));
  };

  // Botón "Guardar" (para guardar datos en caché)
  const handleGuardarLocal = () => {
    // Guardamos la fecha de salida en una key aparte y la precargamos en localStorage
    localStorage.setItem("fechaSalidaPlanta", tiempoSalidaPlanta.fecha || "");
    // Si no se ingresó la fecha, mostramos la advertencia y detenemos el guardado
    ...
    if (!stored) {
      Swal.fire("Error", "No se encontró editDemora en localStorage", "error");
      return;
    }
    const parsed = JSON.parse(stored);
    ...
    // Actualizamos el objeto sin incluir la fecha en tiempoSalidaPlanta
    parsed.procesoFinal = {
      tiempoLlegadaPorteria,
      tiempoSalidaPlanta: {
        hora: tiempoSalidaPlanta.hora,
  ...
  const handleEnviar = async () => {
    ...
    if (confirmResult.isConfirmed) {
      ...
      try {
        ...
        const stored = localStorage.getItem("editDemora");
        if (!stored) throw new Error("No se encontró editDemora en storage");
        const payload = JSON.parse(stored);
        console.log("Payload enviado:", JSON.stringify(payload, null, 2));
        const demoraId = localStorage.getItem("demoraId") || "0";
        const res = await fetch(`/api/demoras/granel/${demoraId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ editDemora: payload }),
        });
        Swal.close();
        if (res.ok) {
  ...
  // Render de cada sección del resumen
